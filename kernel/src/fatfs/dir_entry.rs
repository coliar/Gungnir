...
use super::error::{Error, IoError, ReadExactError};
use super::file::File;
use super::fs::{FatType, FileSystem, OemCpConverter, ReadWriteSeek};
use super::io::{self, Read, ReadLeExt, Write, WriteLeExt};
...
impl<'a, IO: ReadWriteSeek, TP, OCC: OemCpConverter> DirEntry<'a, IO, TP, OCC> {
    ...
    #[cfg(feature = "alloc")]
    #[must_use]
    pub(crate) fn short_file_name(&self) -> String {
    ...
    #[must_use]
    pub(crate) fn short_file_name_as_bytes(&self) -> &[u8] {
    ...
    #[cfg(feature = "lfn")]
    #[must_use]
    pub(crate) fn long_file_name_as_ucs2_units(&self) -> Option<&[u16]> {
    ...
    #[must_use]
    pub(crate) fn to_file(&self) -> File<'a, IO, TP, OCC> {
    ...
    #[must_use]
    pub(crate) fn to_file_with_context(&self, context: FileContext) -> File<'a, IO, TP, OCC> {
        assert!(!self.is_dir(), "Not a file entry");
    ...
    /// Will panic if this is not a file.
    #[must_use]
    pub(crate) fn try_to_file_with_context(&self, context: FileContext) -> Result<File<'a, IO, TP, OCC>, Error<IO::Error>> {
        assert!(!self.is_dir(), "Not a file entry");
    ...
    #[must_use]
    pub(crate) fn to_dir(&self) -> Dir<'a, IO, TP, OCC> {

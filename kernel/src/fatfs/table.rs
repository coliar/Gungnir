...
use super::io::{self, IoBase, Read, ReadLeExt, Seek, Write, WriteLeExt};
...
where
    ...
    Error<E>: From<S::Error> + From<ReadExactError<S::Error>>,
{
    debug!("write FAT - cluster {} value {:?}", cluster, value);
    match fat_type {
        ...
        Error<E>: From<S::Error> + From<ReadExactError<S::Error>>,
    {
        ...
        Ok(match val {
            ...
            n if (0x0FFF_FFF7..=0x0FFF_FFFF).contains(&cluster) => {
                ...
                error!("cluster number {} is a special value in FAT to indicate {}; hiding potential FAT chain value {} and instead reporting as a bad sector", cluster, tmp, n);
                FatValue::Bad // avoid accidental use or allocation into a FAT chain
            }
            n => FatValue::Data(n),
        })

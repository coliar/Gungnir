...
use super::io::{self, IoBase, Read, Seek, SeekFrom, Write};
...
pub(crate) struct Dir<'a, IO: ReadWriteSeek, TP, OCC> {
...
impl<'a, IO: ReadWriteSeek, TP, OCC> Dir<'a, IO, TP, OCC> {
    ...
    pub(crate) fn iter(&self) -> DirIter<'a, IO, TP, OCC> {
...
impl<'a, IO: ReadWriteSeek, TP: TimeProvider, OCC: OemCpConverter> Dir<'a, IO, TP, OCC> {
    ...
    pub(crate) async fn exists(&self, path: &str) -> Result<bool, Error<IO::Error>> {
        self.exists_internal(path, None).await
    }

    /// Check to see if a file with the given name exists
    pub(crate) async fn file_exists(&self, path: &str) -> Result<bool, Error<IO::Error>> {
        self.exists_internal(path, Some(false)).await
    }

    /// Check to see if a directory with the given name exists
    pub(crate) async fn dir_exists(&self, path: &str) -> Result<bool, Error<IO::Error>> {
    ...
    pub(crate) async fn open_dir(&self, path: &str) -> Result<Self, Error<IO::Error>> {
        debug!("Dir::open_dir {}", path);
    ...
    /// * `Error::Io` will be returned if the underlying storage object returned an I/O error.
    pub(crate) async fn open_meta(&self, path: &str) -> Result<DirEntry<'a, IO, TP, OCC>, Error<IO::Error>> {
        debug!("Dir::open_meta {}", path);
        let mut split = split_path(path);
        let mut e = self.clone();
        loop {
            let (name, rest_opt) = split;
    ...
    /// * `Error::InvalidInput` will be returned if `path` points to a file that is a directory.
    /// * `Error::Io` will be returned if the underlying storage object returned an I/O error.
    pub(crate) async fn open_file(&self, path: &str) -> Result<File<'a, IO, TP, OCC>, Error<IO::Error>> {
        debug!("Dir::open_file {}", path);
        let mut split = split_path(path);
        let mut e = self.clone();
        loop {
            let (name, rest_opt) = split;
    ...
    /// * `Error::Io` will be returned if the underlying storage object returned an I/O error.
    pub(crate) async fn create_file(&self, path: &str) -> Result<File<'a, IO, TP, OCC>, Error<IO::Error>> {
        debug!("Dir::create_file {}", path);
        let mut split = split_path(path);
        let mut e = self.clone();
        loop {
            let (name, rest_opt) = split;
    ...
    /// * `Error::Io` will be returned if the underlying storage object returned an I/O error.
    pub(crate) async fn create_dir(&self, path: &str) -> Result<Self, Error<IO::Error>> {
        debug!("Dir::create_dir {}", path);
        let mut split = split_path(path);
        let mut e = self.clone();
        loop {
            let (name, rest_opt) = split;
    ...
    pub(crate) async fn is_empty(&self) -> Result<bool, Error<IO::Error>> {
        debug!("Dir::is_empty");
    ...
    /// * `Error::Io` will be returned if the underlying storage object returned an I/O error.
    pub(crate) async fn remove(&self, path: &str) -> Result<(), Error<IO::Error>> {
        debug!("Dir::remove {}", path);
    ...
    pub(crate) async fn rename(
        ...
        debug!("Dir::rename {} {}", src_path, dst_path);
    ...
    async fn rename_internal(
        ...
        debug!("Dir::rename_internal {} {}", src_name, dst_name);
        // find existing file
...
pub(crate) struct DirIter<'a, IO: ReadWriteSeek, TP, OCC> {
    stream: DirRawStream<'a, IO, TP, OCC>,
    fs: &'a FileSystem<IO, TP, OCC>,
    skip_volume: bool,
    err: bool,
...
impl<'a, IO: ReadWriteSeek, TP: TimeProvider, OCC> DirIter<'a, IO, TP, OCC> {
    ...
    pub(crate) async fn next(&mut self) -> Option<Result<DirEntry<'a, IO, TP, OCC>, Error<IO::Error>>> {
        if self.err {
            return None;
        }
    ...
    #[cfg(feature = "alloc")]
    pub(crate) async fn collect(&mut self) -> Vec<Result<DirEntry<'a, IO, TP, OCC>, Error<IO::Error>>> {
        let mut v = Vec::new();
